...
class PurchasedPlanRepo {
  ...
  Future<ResponseModel> getPurchasedPlanData(int page) async {
    String url =
        "${UrlContainer.baseUrl}${UrlContainer.purchasedPlanEndPoint}?page=$page";
  ...
  Future<dynamic> getPlanPaymentMethod() async {
    String url =
        "${UrlContainer.baseUrl}${UrlContainer.planPaymentMethodEndPoint}";
    ResponseModel responseModel = await apiClient
        .request(url, method.Method.getMethod, null, passHeader: true);

    if (responseModel.statusCode == 200) {
      PlanPaymentMethodResponseModel model =
          PlanPaymentMethodResponseModel.fromJson(
  ...
  Future<ResponseModel> insertPayment(
      {required String order,
      required String amount,
      required String methodCode,
      required String currency}) async {
    String url =
        "${UrlContainer.baseUrl}${UrlContainer.planPaymentInsertEndPoint}";
    ...
    ResponseModel responseModel = await apiClient
        .request(url, method.Method.postMethod, map, passHeader: true);
    return responseModel;
  }
}
