...
class BuyPlanController extends GetxController {
  ...
  Future<void> loadBuyPlanData() async {
    ...
    if (responseModel.statusCode == 200) {
      ...
      if (model.status.toString().toLowerCase() == MyStrings!.success.toLowerCase()) {
        paymentSystemList[1] = "${MyStrings.fromBalance} ($currencySym${MyConverter.twoDecimalPlaceFixedWithoutRounding(model.data?.balance ?? "0")})";
        paymentSystemList[2] = "${MyStrings.fromProfitWallet} ($currencySym${MyConverter.twoDecimalPlaceFixedWithoutRounding(model.data?.profitBalance ?? "0")})";
  ...
  Future<void> planPurchase({required int planId}) async {
    ...
    String paymentMethod = method == "3" // 1 -> balance | 2 -> gateway | 3 -> profit wallet
        ? "2"
        : method == "2"
            ? "3"
            : method;
